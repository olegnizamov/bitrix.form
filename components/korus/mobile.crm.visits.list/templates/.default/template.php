...
$APPLICATION->IncludeComponent("bitrix:mobile.interface.grid", "", array(
	...
	"AJAX_PAGE_PATH" => "/mobile/crm/visits/?page=list",
	"SHOW_SEARCH" => "N"
));

$arJsParams = array(
	"ajaxPath" => $arResult["AJAX_PATH"],
	"sortPath" => "/mobile/crm/visits/?page=sort",
	"fieldsPath" => "/mobile/crm/visits/?page=fields",
	"filterPath" => "/mobile/crm/visits/?page=filter",
	"filterAjaxPath" => "/mobile/?mobile_action=mobile_grid_filter",
	"contextMenuTitle" => $arResult["CURRENT_FILTER"] !== "all" && $arResult['FILTER_PRESETS'][$arResult["CURRENT_FILTER"]]["name"] ? $arResult['FILTER_PRESETS'][$arResult["CURRENT_FILTER"]]["name"] : GetMessageJS("M_CRM_VISITS_LIST_TITLE")
);
?>
<script>
	BX.ready(function(){
		...
		var customItems = [
			...
			{
				name: '<?=GetMessageJS("M_CRM_VISITS_ADD")?>',
				image: "/bitrix/js/mobile/images/plus.png",
				action: function()
				{
					BXMobileApp.PageManager.loadPageModal({
						url: "<?=CUtil::JSEscape($arParams["VISITS_CREATE_URL_TEMPLATE"])?>"
					});
				}
			},
			<?endif?>
		...
		BX.Mobile.Crm.Deal.ListConverter.init(<?=CUtil::PhpToJSObject(array(
			...
			"ajaxConvertPath" => '/mobile/?mobile_action=mobile_crm_visits_actions'
		))?>);
	});
</script>