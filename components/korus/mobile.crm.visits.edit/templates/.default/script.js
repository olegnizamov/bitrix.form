...
BX.Mobile.Crm.Deal.Edit = {
	...
	submit: function()
	{
		...
		if(form)
		{
			...
			{
				dataFormValues["PROPERTY_KLIENT_KONTAKTNOE_LITSO"] = this.contactEditor.curEntityId;
			}
			if (this.companyEditor)
			{
				dataFormValues["PROPERTY_KLIENT_KOMPANIYA"] = this.companyEditor.curEntityId;
			}
			...
			BX.ajax({
				...
				onsuccess: BX.proxy(function(json)
				{
					...
					else
					{
						...
						if (this.mode == "CREATE")
						{
							var viewPath = this.dealViewPath.replace("#visits_id#", json.itemId);
