...
$mode = $arResult['MODE'];
$formTitle = GetMessageJS("M_CRM_VISITS_EDIT_{$mode}_TITLE");
$formId = "MOBILE_CRM_VISITS_EDIT_".$arResult['ELEMENT_ID'];
$ajaxPath = '/mobile/?mobile_action=mobile_crm_visits_actions';
...
$arJsParams = array(
	...
	"formId" => $formId,
	"dealViewPath" => $arParams['VISITS_VIEW_URL_TEMPLATE'],
	"mode" => $arResult['MODE'],
...
<script>
	...
	var menu = new BXMobileApp.UI.Menu({
		items: [
			...
			{
				name: '<?=GetMessageJS("M_CRM_VISITS_MENU_DELETE")?>',
	...
	window.BXMobileApp.UI.Page.TopBar.updateButtons({
		ok: {
			...
			name: "<?=($arResult['MODE'] == "CONVERT" ? GetMessageJS("M_CRM_VISITS_EDIT_CONTINUE_BTN") : GetMessageJS("M_CRM_VISITS_EDIT_SAVE_BTN"))?>",
			bar_type: "navbar",
			position: "right"
		}
	});
	...
	<?if (isset($arResult['CONVERSION_LEGEND']) && !empty($arResult['CONVERSION_LEGEND'])):?>
		app.alert({title: "<?=GetMessageJS("M_CRM_VISITS_CONVERSION_NOTIFY")?>", text: "<?=CUtil::JSEscape($arResult['CONVERSION_LEGEND'])?>"});
	<?endif?>
</script>
