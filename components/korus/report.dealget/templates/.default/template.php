...
<table cellspacing="0" class="reports-list-table">
    ...
    <tr>
        <?php foreach ($arResult['TABLE'] as $id => $data) { ?>
            <?php foreach ($data as $key => $val) { ?>
                <th class="reports-first-column reports-head-cell-top" style="white-space: nowrap;"> <?= $val['head'] ?></th>
            <?php } ?>
            <?php break ?>
        <?php } ?>
    </tr>
    ...
    <?php foreach ($arResult['TABLE'] as $id => $data) { ?>
        ...
        <?php foreach ($data as $key => $val) { ?>
            <?php
            if ($key == 'TITLE') {
                ?><td class="reports-first-column" style="white-space: nowrap;"> <a href="/crm/deal/show/<?= $id ?>/"><?= $val['data'] ?></a></td> <?php
                continue;
            }
            ?>
            <td style="white-space: nowrap;"> <?= $val['data'] ?></td>
        <?php } ?>
        </tr>
    <?php } ?>
    </tbody>
    ...
    <div class="sidebar-block-inner">
        <form method="get">
            ...
            <div class="filter-field filter-field-type chfilter-field-TYPE_ID" callback="RTFilter_chooseBoolean">
                <label for="stages" class="filter-field-title">Стадии сделок</label>
                <select id="stages" name="STAGES[]" style="width:100%" caller="true" size="10" multiple>
                    <?php foreach ($arResult['DIRECTION'] as $direction => $stages) {?>
                    <optgroup label=" <?= $direction ?>">
                    <?php foreach($stages as $stage => $name):?>
                                <option value="<?=$stage?>" <?= in_array($stage, $_GET['STAGES']) ? 'selected' : '' ?>><?=$name?></option>
                    <?php endforeach ?>
                        <?php } ?>
                </select>
            </div>
